 ...
 * @param handle
 *
 * @return const char * Pointer to file name string.
 */
const char * gnuplot_tmpfile(gnuplot_ctrl * handle);
 ...
 * @param title
 */
void gnuplot_plot_atmpfile(gnuplot_ctrl * handle, const char* tmp_filename, const char* title);
...
{
    const char *    cmd    = (handle->nplots > 0) ? "replot" : "plot";
...
{
    const char *    cmd    = (h->nplots > 0) ? "replot" : "plot";
    const char *lpts_title  = (title == NULL)      ? "(none)" : title;

    gnuplot_cmd(h, "%s %s title \"%s\" with %s",
                  cmd, equation, lpts_title, h->pstyle) ;
    h->nplots++ ;
    return ;
}
...
{	
#if defined(_WIN32) || defined(_WIN64) 
	 static const char * tmp_filename_template = "gnuplot_tmpdatafile_XXXXXX";
#else
    int                 unx_fd;
    static const char * tmp_filename_template = "/tmp/gnuplot_tmpdatafile_XXXXXX";  
#endif //#if defined(_WIN32) || defined(_WIN64)
...
////const int    li_is3D /* O is 2D  1 is 3D */
...
{
  //const char *    cmd    = (handle->nplots > 0) ? "replot" : (li_is3D)? "splot" : "plot";
  const char    *cmd    = (handle->nplots > 0) ? "replot" : "plot";
  title                 = (title == NULL)      ? "(none)" : title;
