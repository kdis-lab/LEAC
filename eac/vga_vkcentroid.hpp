/*! \file vga_vkcentroid.hpp
 *
 * \brief VGA \cite Bandyopadhyay:Maulik:GACVarK:VGA:2002
 *
 * \details This file is part of the LEAC.\n\n
 * Implementation of the VGA algorithm based on the paper:\n
 * S. Bandyopadhyay and U. Maulik, "Nonparametric genetic clustering: 
 * comparison of validity indices," in IEEE Transactions on Systems, 
 * Man, and Cybernetics, Part C (Applications and Reviews), vol. 31, no. 1, 
 * pp. 120-125, Feb. 2001, doi: 10.1109/5326.923275.
 * <a href="https://ieeexplore.ieee.org/document/923275</a>,\n
 * <a href="https://doi.org/10.1109/5326.923275</a>.\n
 * \n
...
#ifndef __VGA_VKCENTROID_HPP__
#define __VGA_VKCENTROID_HPP__
...
/*! \fn mat::MatrixRow<T_FEATURE> vga_vkcentroid(inout::OutParamGAC<T_REAL,T_CLUSTERIDX> &aoop_outParamGAC, inout::InParamPcPmVk<T_CLUSTERIDX,T_REAL,T_FEATURE,T_FEATURE_SUM,T_INSTANCES_CLUSTER_K> &aiinp_inParamPcPmVk, const INPUT_ITERATOR aiiterator_instfirst, const INPUT_ITERATOR aiiterator_instlast, dist::Dist<T_REAL,T_FEATURE> &aifunc2p_dist) 
  \brief VGA \cite Bandyopadhyay:Maulik:GACVarK:VGA:2002
  \details Implementation of the VGA algorithm based on \cite Bandyopadhyay:Maulik:GACVarK:VGA:2002. Which automatically finds K cluster using the Davies–Bouldin index.
 ...
 )
{
  gaencode::ChromVariableLength<T_FEATURE,T_REAL>
    lochrom_best
    ( measuare_undefObjetiveFunc(T_REAL),
      measuare_undefObjetiveFunc(T_REAL)
    );
   ...
   */
  std::vector<gaencode::ChromVariableLength<T_FEATURE,T_REAL> >
...
#ifdef __VERBOSE_YES
  ++geiinparam_verbose;
  const char* lpc_labelAlgGA = "vga_vkcentroid:";
  ...
  {/*BEGIN CREATE SPACE FOR STORE POPULATION*/
	...
	lvectorchrom_population.push_back
	  (gaencode::ChromVariableLength<T_FEATURE,T_REAL>
	   ( uintidx(uniformdis_mmcidxMinMaxK(gmt19937_eng)) * data::Instance<T_FEATURE>::getNumDimensions() )
	   );
      }
  ...
  */    
  { /*BEGIN POPULATION INITIALIZATION*/
	...
	liter_iChrom.getStringSize() / data::Instance<T_FEATURE>::getNumDimensions();
	...
	(lmatrixrowt_centroidsChrom, 
	 ...
	 aiiterator_instlast
      ...
      liter_iChrom.setFitness( measuare_undefIndexI(T_REAL) );	
      liter_iChrom.setObjetiveFunc( measuare_undefObjetiveFunc(T_REAL) );	
  ...
  while ( 1 ) {
    ...
    */
	...
	if (  lui_numClusterK >= 2 )  { 
	  if  ( liter_iChrom.getObjetiveFunc() == measuare_undefObjetiveFunc(T_REAL) ) {
	    /*DECODE CHROMOSOME*/
	    mat::MatrixRow<T_FEATURE> 
	      lmatrixrowt_centroidsChrom
	      (lui_numClusterK,
		...
		(lmatrixrowt_centroidsChrom,
		 ...
		 aifunc2p_dist
	      ...
	      liter_iChrom.setObjetiveFunc(lm_indexI); 
	      liter_iChrom.setFitness(lm_indexI);
	    } /* lmcidx_numClusterNull == 0 */
	    else {
	      liter_iChrom.setObjetiveFunc(measuare_undefIndexI(T_REAL)); 
	      liter_iChrom.setFitness(measuare_undefIndexI(T_REAL));
	...
	(lvectorchrom_population.begin(), 
	 ...
	 [](const gaencode::ChromVariableLength<T_FEATURE,T_REAL>& x, 
	    const gaencode::ChromVariableLength<T_FEATURE,T_REAL>& y
	    ) 
	...
	const std::vector<T_REAL>&& lvectorT_probDistRouletteWheel =
	  ...
	  (lvectorchrom_population.begin(),lvectorchrom_population.end(),
	   [](const gaencode::ChromVariableLength<T_FEATURE,T_REAL>& lchrommatrixwrn_iter)
	   -> T_REAL
	   {
	...
	//   (gaencode::ChromVariableLength<T_FEATURE,T_REAL>(lochrom_best));
	...
	gaiterator::crossover
	  (lvectorchrom_matingPool.begin(),
	   ...
	   lvectorchrom_population.begin(),
	   lvectorchrom_population.end(),
	   [&](const gaencode::ChromVariableLength<T_FEATURE,T_REAL>&
	       aichrom_parent1,
	       const gaencode::ChromVariableLength<T_FEATURE,T_REAL>&
	       aichrom_parent2,
	       gaencode::ChromVariableLength<T_FEATURE,T_REAL>&
	       ...
	       )
	   {
	     ...
	     if ( uniformdis_real01(gmt19937_eng) <
		  aiinp_inParamPcPmVk.getProbCrossover()  ) {
	       ...
	       gaclusteringop::onePointIndivisibleCrossover
	       (aochrom_child1,
		aochrom_child2,
		aichrom_parent1,
		aichrom_parent2
	     ...
	     aochrom_child1.setObjetiveFunc( measuare_undefObjetiveFunc(T_REAL) );	
	     aochrom_child1.setFitness( measuare_undefIndexI(T_REAL) );	
	     aochrom_child2.setObjetiveFunc( measuare_undefObjetiveFunc(T_REAL) );	
	     aochrom_child2.setFitness(measuare_undefIndexI(T_REAL) );	
	...
	for ( auto&& liter_iChrom : lvectorchrom_matingPool ) {
	  if ( uniformdis_real01(gmt19937_eng) 
	       < aiinp_inParamPcPmVk.getProbMutation() ) {
	    //IF BEGIN  MUTATION
	    garealop::randomMutation(liter_iChrom);	   
	    liter_iChrom.setFitness(measuare_undefIndexI(T_REAL) );	
	    liter_iChrom.setObjetiveFunc(measuare_undefObjetiveFunc(T_REAL) );	
	  } //END BEGIN  MUTATION
	}
